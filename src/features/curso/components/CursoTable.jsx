...
import { CrearActualizarCursoModal } from '../modals/CrearActualizarCursoModal';
import { CursoContext } from '../context/CursoContextProvider';

export const CursoTable = React.memo(({
    filtros={},
}) => {
    ...
    const [cursoDelete, setCursoDelete] = useState(0);
    const [isCreate, setIsCreate] = useState(true);
    const [selectedRowKeys, setSelectedRowKeys] = useState([]);
    const [selectedRowValues, setSelectedRowValues] = useState([]);
    const { deleteCurso, contextHolder, contextHolder2, isLoading, cursos, consultaCursos} = useContext(CursoContext);
    ...
    const eliminar = async()=>{
        ...
        if(isDeleteMasivo){
            ...
            value = {ids:[cursoDelete]}
        }
        const response = await deleteCurso(value);
        if(response.ok){
            reiniciarVariables();
            await consultaCursos();
        }
    }
    ...
    const handlePageChange = async (page, pageSize) => {
        ...
        if (page !== cursos?.current_page || pageSize !== cursos?.per_page) {
            await consultaCursos(values);
        }
    };
  ...
  return (
    <>
            ...
            <Row>
                <Col span={24}>
                    <Table 
                        ...
                        columns={columns} 
                        dataSource={cursos?.data} 
                        size="small"
                        scroll={{
                            x: 500
                        }}
                        pagination={{
                            current: cursos?.current_page ?? 1,
                            pageSize: cursos?.per_page ?? 5,
                            total: cursos?.total ?? 0,
        ...
        {/* componentes modales */}
        <CrearActualizarCursoModal
            open={modalCrearActualizar}
            toggle={toggleModalCrearActualizar}
            curso={cursoEdit}
            isCreate={isCreate}
            titulo={isCreate ? 'Crear Curso' : 'Modificar Curso'}
        />
